...
class LocalCommandExecutor:
    ...
    def prepare_burst_input_parent(self, slc_dirs: list[Path], burst_dir: str) -> Path | str:
        burst_path = Path(burst_dir).expanduser()
        staging_path = burst_path.parent / ".sar_agent_burst_input"
        marker_path = staging_path / ".sar_agent_generated"

        if staging_path.exists() and not staging_path.is_dir():
            return f"burst 输入暂存路径已存在但不是目录：{staging_path}"

        staging_path.mkdir(parents=True, exist_ok=True)

        existing_items = list(staging_path.iterdir())
        if existing_items and not marker_path.exists():
            return (
                "burst 输入暂存目录已存在，但缺少本程序标记文件，"
                f"为避免误删用户文件，请人工检查：{staging_path}"
            )
        ...
        for child in existing_items:
            ...
            if child.is_symlink() or child.is_file():
                ...
                continue
            return (
                "burst 输入暂存目录包含非链接/非文件内容，"
                f"为避免误删用户文件，请人工检查：{child}"
            )

        marker_path.write_text("Generated by SAR/GAMMA assistant.\n", encoding="utf-8")

        for slc_dir in slc_dirs:
            link_path = staging_path / slc_dir.name
            link_path.symlink_to(slc_dir.resolve(), target_is_directory=True)
    ...
    def run_extract_burst_multi_from_unzip_dir(
        ...
        burst_input_parent = self.prepare_burst_input_parent(slc_dirs, burst_dir)
        if isinstance(burst_input_parent, str):
            return burst_input_parent

        found_dates = ", ".join(path.name for path in slc_dirs)
        result = self.run_extract_burst_multi(
            unzip_dir=str(burst_input_parent),
            burst_dir=burst_dir,
        ...
        return (
            f"burst 输入父目录：{burst_input_parent}\n"
            f"已纳入 SLC 日期目录：{found_dates}\n\n"
            f"{result}"
        )
    ########################地理编码############
    def run_slc_geo(
